...
static inline void EncodeFeatureWeight(const string &featname, weight_t weight, ostream *output) {
  ...
  string b64;
  {
    ostringstream base64_strm;
    {
      ...
      for (SparseVector<weight_t>::const_iterator it = vec.begin();
           it != vec.end(); ++it)
        if (it->second != 0) EncodeFeatureWeight(FD::Convert(it->first), it->second, &strm);
