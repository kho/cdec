...
struct SoftSynFeatureImpl{
	...
	void InitializeInputSentence(const std::string& parse_file, const std::string& index_map_file) {
		FreeSentenceVariables();
		parsed_tree_ = ReadParseTree(parse_file);

		if (index_map_file != "")
			ReadIndexMap(index_map_file);
	...
	void MapIndex(short int begin, short int end, short int& mapped_begin, short int& mapped_end) {
		if (index_map_ == NULL) {
			mapped_begin = begin;
			mapped_end = end;
			return;
	 ...
	 */
	void FindConsts(const SParsedTree *tree, int begin, int end, vector<STreeItem*>& consts) {
		STreeItem *item;
		item = tree->m_vecTerminals[begin]->m_ptParent;
		while (true) {
			while (item->m_ptParent != NULL && item->m_ptParent->m_iBegin == item->m_iBegin && item->m_ptParent->m_iEnd <= end)
				item = item->m_ptParent;

			if (item->m_ptParent == NULL && item->m_vecChildren.size() == 1 && strcmp(item->m_pszTerm, "ROOT") == 0)
				item = item->m_vecChildren[0]; //we automatically add a "ROOT" node at the top, skip it if necessary.
	 ...
	 * a constituent XP is crossed only if the span not only covers parts of XP's content, but also covers one or more words outside XP
	 * a span may have X+, Y+
	 *
	 * (note, we refer X* features to X= features in Marton & Resnik (2008))
	 */
	void GenerateSoftFeature(int begin, int end, const SParsedTree *tree, vector<string>& vecFeature) {
			...
			for (size_t i = 0; i < vecNode.size(); i++) {
				...
				while (item != lcp) {
					...
					if (item->m_iBrotherIndex > 0
						&& item->m_ptParent->m_vecChildren[item->m_iBrotherIndex - 1]->m_iBegin >= begin
						&& item->m_ptParent->m_vecChildren[item->m_iBrotherIndex - 1]->m_iEnd <= end)
	...
	void GenerateSoftFeatureFromFlattenedTree(int begin, int end, const SParsedTree *tree, vector<string>& vecFeature) {
		vector<STreeItem*> vecNode;
		FindConsts(tree, begin, end, vecNode);
		...
		if (vecNode.size() == 1) {
			...
			for (i = 1; i < vecNode.size(); i++) {
				if (vecNode[i]->m_ptParent != vecNode[0]->m_ptParent)
					break;
			}
			if (i == vecNode.size()) {
				//they share a common parent
				string feature_name = string(vecNode[0]->m_ptParent->m_pszTerm) + string("&");
				vecFeature.push_back(feature_name);
			} else {
	...
	void SetSoftSynFeature(const Hypergraph::Edge& edge, SparseVector<double>* features, const vector<const void*>& ant_states, void* state) {vector<uint16_t> vec_pos;
		...
		const vector<string> vecFeature = GenerateSoftFeature(mapped_begin, mapped_end, map_features_);
		for (size_t i = 0; i < vecFeature.size(); i++) {
			int f_id = FD::Convert(vecFeature[i]);
			if (f_id)
				features->set_value(f_id, 1);
		}
	}

private:
	const vector<string>& GenerateSoftFeature(int begin, int end, MapFeatures *map_features) {
		string key;
...
void SoftSynFeature::TraversalFeaturesImpl(const SentenceMetadata& /* smeta */,
...
boost::shared_ptr<FeatureFunction> CreateSoftSynFeatureModel(std::string param) {
	SoftSynFeature *ret = new SoftSynFeature(param);
	return boost::shared_ptr<FeatureFunction>(ret);
}

boost::shared_ptr<FeatureFunction> SoftSynFeatureFactory::Create(std::string param) const {
...
struct SoftKBestSynFeatureImpl{
	...
	void SetSoftKBestSynFeature(const Hypergraph::Edge& edge, SparseVector<double>* features, const vector<const void*>& ant_states, void* state) {
		...
		//soft feature for the whole span
		const MapDouble* pMapFeature = GenerateSoftFeature(mapped_begin, mapped_end, map_features_);
		for (MapDouble::const_iterator iter = pMapFeature->begin(); iter != pMapFeature->end(); iter++) {
			int f_id = FD::Convert(iter->first);
			if (f_id)
				features->set_value(f_id, iter->second);
		}
	}
...
private:
	...
	void ReadIndexMap(const std::string& index_map_file) {
		...
		index_map_[i] = ix;
		assert(vec_parsed_tree_.size() == 0 || ix == vec_parsed_tree_[0]->m_vecTerminals.size());
		delete reader;
	}
	 ...
	 * (note, we refer X* features to X= features in Marton & Resnik (2008))
	 */
	void GenerateSoftFeature(int begin, int end, const vector<SParsedTree *>& vec_tree, const vector<double>& vec_prob, MapDouble* pMapFeature) {
	...
	const MapDouble* GenerateSoftFeature(int begin, int end, MapDoubleFeatures *map_features) {
		string key;
		GenerateKey(begin, end, key);
	...
	void InitializeFeatures(MapDoubleFeatures *map_features) {
		...
		for (size_t i = 0; i < vec_tree_prob_.size(); i++) {
			vec_prob.push_back(exp10(vec_tree_prob_[i] - vec_tree_prob_[0]));
			tmp += vec_prob[i];
		}
	...
	void FreeSentenceVariables( ) {
		for (size_t i = 0; i < vec_parsed_tree_.size(); i++) {
			if (vec_parsed_tree_[i] != NULL)
				delete vec_parsed_tree_[i];
		}
		...
		if (map_features_ != NULL) {
			for (MapDoubleFeatures::iterator iter = map_features_->begin(); iter != map_features_->end(); iter++)
				delete iter->second;
			delete map_features_;
		}
	}
	...
	void ReadParseTree(const std::string& parse_file, vector<SParsedTree*>& vec_tree, vector<double>& vec_prob) {
	...
	void ReadParseTree2(const std::string& parse_file, vector<SParsedTree*>& vec_tree, vector<double>& vec_prob) {
		SParseReader *reader = new SParseReader(parse_file.c_str(), false);
		double prob;
		SParsedTree *tree;
...
SoftKBestSynFeature::~SoftKBestSynFeature( ) {
	delete pimpl_;
}
...
void SoftKBestSynFeature::TraversalFeaturesImpl(const SentenceMetadata& /* smeta */,
                                          const Hypergraph::Edge& edge,
                                          const vector<const void*>& ant_states,
                                          SparseVector<double>* features,
                                          SparseVector<double>* estimated_features,
...
boost::shared_ptr<FeatureFunction> CreateSoftKBestSynFeatureModel(std::string param) {
	SoftKBestSynFeature *ret = new SoftKBestSynFeature(param);
	return boost::shared_ptr<FeatureFunction>(ret);
}

boost::shared_ptr<FeatureFunction> SoftKBestSynFeatureFactory::Create(std::string param) const {
